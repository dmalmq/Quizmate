class Quizzes::ChallengesController < ApplicationController
  # def index
  ...
  def perform(challenge)
    ...
    if challenge.user_option_id == challenge.question.correct_option_id
      ...
      challenge.question.corrected = true
    else
      challenge.question.streak = 0
      challenge.question.corrected = false
      challenge.question.total_asked += 1
      challenge.question.last_asked = Time.now.strftime("%d/%m/%Y %H:%M")
    end
    challenge.question.save # saving the question with the updated attribute
  end
end
